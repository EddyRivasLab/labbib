    ...
    if (@tokens == 0) {
        ...
        if ( $tokens[0] =~ /(^|\s)[[:lower:]]/) { # name has von part or has only lowercase names
            ...
            if ( $tokens[0] !~ /\{/ && $tokens[0] =~ /^((.*)\s+)?\b(\S+)$/) {
                ...
                # if there are more than two name parts
                if (scalar @name_parts > 2) {
                  @name_parts = grep { !/^$/ } @name_parts;
                # find all the ones with a single letter followed by a . and place them in
                # the same field.
                  $first = join ' ', @name_parts[0..$#name_parts-1];
                }
                return ($first, undef, $name_parts[-1], undef);
            }
        }
...
This software is copyright (c) 2014 by Gerhard Gossen.
